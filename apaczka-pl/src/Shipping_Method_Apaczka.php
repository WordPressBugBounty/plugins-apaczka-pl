...
class Shipping_Method_Apaczka extends WC_Shipping_Method {
    ...
    const PARCEL_LOCKER_GEOWIDGET_IDS = [
        14  => 'UPS',
        15  => 'UPS',
        16  => 'UPS',
        23  => 'DPD',
    ...
    public function __construct( $instance_id = 0 ) {
        ...
        $this->method_description
            = __( ' Register on <a href="https://www.apaczka.pl/?register=1&register_promo_code=WooCommerce" target="_blank">www.apaczka.pl &rarr;</a>',
            'apaczka-pl' );
        ...
        add_action( 'admin_footer', [ $this, 'cancel_package_popup' ] );
    ...
    public function woocommerce_review_order_after_shipping() {
        if ( $this->is_delivery_map_button_display() && ! self::$review_order_after_shipping_once ) {
            ...
            wc_get_template(
                ...
                [ 'point_id' => $point_id, 'only_cod' => $only_cod ],
                '',
    ...
    /**
     * @param int $apaczka_order_id
     *
     * @return string
     */
    public static function get_apaczka_order_status( int $apaczka_order_id
    ): ?string {
    ...
    public static function order_metabox_content(
        ...
        $apaczka_wc_order_data = self::set_defaults_to_wc_order_data( $apaczka_wc_order_data,
            $order );
            ...
            if ( isset( $apaczka_wc_order_data['apaczka_response']->order->id ) ) {
                ...
                if ( 'CANCELLED' === $status ) {
                    ...
                    if( $order->get_date_paid() ) {
                        ...
                        if( 'on-hold' !== $current_order_status ) {
    ...
    private static function declared_value_validate(
        ...
        if ( $cod_amount > 0 ) {
            if ( $declared_value === 0.0 ) {
                ( new Alerts() )->add_error( __( 'The Declared value field cannot be empty',
                    'apaczka-pl' ), $api_context );

                return false;
            }
            if ( $declared_value < $cod_amount ) {
                ( new Alerts() )->add_error( __( 'The Declared Value field must be equal to or higher than the COD amount',
                    'apaczka-pl' ), $api_context );
    ...
    public static function download_turn_in() {
        ...
        try {
            $apaczka_response  = ( new Web_Api_V2() )->turn_in( [ $apaczka_id ] );
            $response_messages = ( new Alerts() )->get_alerts_unformatted_by_context( 'cancel_order' );
    ...
    public static function ajax_calculate_package() {
        ...
        $apaczka_order = self::create_api_order( $order, $data,
        ...
        try {
            ...
            if ( ! empty( $response_messages['error'] ) ) {
                ...
                foreach ( $services as $service ) {
                    $services_cache[ $service->service_id ] = [
                ...
                foreach ( $price_table as $k => $price_item ) {
                    ...
                    $return_price_table[ $k ] =
                        [
                            ...
                            'price_gross'    => [ $price_item->price_gross ][0],
        ...
        if ( key_exists( $default_service, $return_price_table ) ) {
            $copy = [ $default_service => $return_price_table[ $default_service ] ];
    ...
    public static function ajax_create_package() {
        ...
        $apaczka_order = self::create_api_order( $order, $data,
            $shipping_method, $apaczka_delivery_point,
        ...
        if ( is_object( $apaczka_response )
            ...
            $ret['error_messages']     = '';
            $ret['message']            = __( 'Package created',
                'apaczka-pl' );
            $ret['status']             = 'ok';
    ...
    public function process_admin_options() {
        ...
        $this->check_apaczka_maps_plugin(
            (int) $this->get_field_value( 'service',
                $instance_form_fields['service'],
                $post_data )
        );

        $create_template = $this->get_field_value( 'create_template',
                $instance_form_fields['create_template'],
                $post_data ) === 'yes';

        $new_template_name = $this->get_field_value( 'new_template_name',
        ...
        if ( $create_template ) {
            ( new Gateway_Settings_Templates_Helper() )->create( $created_template,
                $new_template_name );
        }

        return update_option( $this->get_instance_option_key(),
            apply_filters( 'woocommerce_shipping_' . $this->id . '_instance_settings_values',
                $this->instance_settings, $this ), 'yes' );
    }
    ...
    public static function settings_field( $settings ) {
        $settings['display_apaczka_map'] = [
            'title'       => esc_html__( 'Apaczka.pl Delivery Points Map',
                'apaczka-pl' ),
            'type'        => 'checkbox',
            'description' => __( 'Displays a map of delivery points on the checkout form for this shipping method.',
                'apaczka-pl' ),
            'desc_tip'    => true,
            'default'     => '',
        ];

        $settings['supplier_apaczka_map'] = [
            'title'   => esc_html__( 'Apaczka.pl Supplier',
                'apaczka-pl' ),
            'type'    => 'select',
            'default' => 'all',
            'options' => [
                ...
                'PWR'        => __( 'Orlen Paczka',
                    'apaczka-pl-mapa-punktow' ),
                'POCZTA'     => __( 'Poczta Polska',
                    'apaczka-pl-mapa-punktow' ),
                'UPS'        => __( 'UPS', 'apaczka-pl-mapa-punktow' ),
            ],
        ];

        $settings['only_cod_apaczka_map'] = [
            'title'       => esc_html__( 'Apaczka.pl only C.O.D points',
                'apaczka-pl' ),
            'type'        => 'checkbox',
            'description' => __( 'Displays only points with Collect on Delivery.',
                'apaczka-pl' ),
            'desc_tip'    => true,
            'default'     => '',
        ];
    ...
    public function filtering_shipping_fields() {
        ...
        foreach ( $shipping_methods as $shipping_method ) {
            add_filter( 'woocommerce_shipping_instance_form_fields_' . $shipping_method->id,
                [ $this, 'add_map_field' ] );
        }
    }
        ...
        if ( ! class_exists( 'Apaczka_Points_Map\Points_Map_Plugin' ) ) {
            if( is_checkout() ) {
                if( has_block( 'woocommerce/checkout' ) ) {
                    ...
                    $plugin = new Plugin();
                    wp_enqueue_script($plugin->get_front_blocks_script_id(),
    ...
    private function get_package_properties($order = null) {
        ...
        $package_properties = [
            'dispath_point_ups'      => get_option( $settings->get_setting_id( 'dispath_point_ups' ),
                '' ),
            'dispath_point_kurier48' => get_option( $settings->get_setting_id( 'dispath_point_kurier48' ),
                '' ),
            'dispath_point_inpost'   => get_option( $settings->get_setting_id( 'dispath_point_inpost' ),
                '' ),
            'dispath_point_dpd'      => get_option( $settings->get_setting_id( 'dispath_point_dpd' ),
                '' ),
            'shipping_method'        => get_option( $settings->get_setting_id( 'shipping_method' ),
                '' ),
            'is_nstd'                => get_option( $settings->get_setting_id( 'is_nstd' ),
                '' ),
            'parcel_type'            => get_option( $settings->get_setting_id( 'parcel_type' ),
                '' ),
            'declared_content'       => $declared_amount,
            'cod'                    => $cod_amount,
            'service'                => get_option( $settings->get_setting_id( 'service' ),
                '' ),
            'package_width'          => get_option( $settings->get_setting_id( 'package_width' ),
                '' ),
            'package_depth'          => get_option( $settings->get_setting_id( 'package_depth' ),
                '' ),
            'package_height'         => get_option( $settings->get_setting_id( 'package_height' ),
                '' ),
            'package_weight'         => get_option( $settings->get_setting_id( 'package_weight' ),
                '' ),
            'package_contents'       => get_option( $settings->get_setting_id( 'package_contents' ),
                '' ),
            'cod_amount'             => '',
            'pickup_date'            => '',
            'pickup_hour_from'       => get_option( $settings->get_setting_id( 'pickup_hour_from' ),
                '' ),
            'pickup_hour_to'         => get_option( $settings->get_setting_id( 'pickup_hour_to' ),
                '' ),
        ];
    ...
    public function filter_shipping_methods( $rates ) {
        ...
        if( !empty($rates) && is_array($rates) && count($rates) === 1 && has_block( 'woocommerce/checkout' ) ) {
            ...
            if( is_checkout() ) {
                $plugin = new Plugin();
                wp_enqueue_script('apaczka-single', $plugin->get_plugin_js_url() . '/blocks/single.js', ['jquery']);
                wp_localize_script(
                    'apaczka-single',
                    'apaczka_single',
                    array(
