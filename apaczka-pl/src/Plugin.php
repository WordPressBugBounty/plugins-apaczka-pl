...
class Plugin extends Abstract_Ilabs_Plugin {
    ...
    protected function plugins_loaded_hooks(){
        ...
        if (is_admin()){
            ...
            add_filter( 'woocommerce_shipping_methods',
                [ $this, 'woocommerce_shipping_methods' ], 20, 1 );
        }
    ...
    public static function get_option( $key ) {
        return get_option( Plugin::APP_PREFIX
    ...
    private function init_admin_features() {
        add_action( 'woocommerce_settings_saved', [ $this, 'save_post' ] );
        ...
        if ( ! class_exists( 'Apaczka_Points_Map\Points_Map_Plugin' ) ) {
            add_action( 'woocommerce_admin_order_data_after_shipping_address',
                function ( WC_Order $order ) {
                    $apaczka_delivery_point = get_post_meta( $order->get_id(),
                        'apaczka_delivery_point', true );

                    if ( ! empty( $apaczka_delivery_point ) ) {
                        echo '<div class="order_data_column"><h4>' . __( "Others:",
                                'apaczka-pl' ) . '</h4><p><strong>'
    ...
    public function enqueue_frontend_scripts() {
        if ( ! class_exists( 'Apaczka_Points_Map\Points_Map_Plugin' ) ) {
            if( is_checkout() ) {
				...
				wp_enqueue_style($this->get_admin_css_id(),
